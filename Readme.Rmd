...
We ran two pilot experiments and a main experiment. In the paper, we mostly talk about the main experiment and don't like talking about pilot 1 and 2, much like people avoid talking about [the East Pole and the West Pole](https://standardebooks.org/ebooks/a-a-milne/winnie-the-pooh/text/chapter-9).
...
Participant vocabulary size was established as follows. Each participant responded to nonce words and real words. As described above, real words were distributed across pre-established familiarity bins (from 1, least familiar, to 50, most familiar). Based on participants’ yes and no responses, we identified the highest familiarity bin in which the participant did not know more than one real word.

Here is an example. Bin 6, a relatively unfamiliar bin, has four real words: glosszáz ‘to gloss’, libling ‘favorite’, magmatit ‘magnetite’, viszonvád ‘counter-accusation’. Bin 47, a relatively familiar bin, has barát friend, dolog ‘thing’, törékeny ‘fragile’, karakter ‘character’. Participant A might say "no" to magmatit and libling and glosszáz. We check that hasn't happened in bins 7-50, where they only rejected one word per bin at most. Participant B might say "no" to both barát and dolog (no actual participant did this). This means they rejected more than one word in bin 47. We check that this hasn't happened in bins 48-50 meaning that 47 is the highest bin where they did this. Note that participants see words in random order. Participant A might have responded to magmatit in trial 1, trial 250, or anywhere in between. For more detail, see the online supplement.
...
The correlation between age, vocabulary size, and education can be seen below.

```{r ageeduvoc, fig.width = 9, fig.height = 3}

p1 = d %>% 
  distinct(id,participant_age,participant_vocabulary_size,edu) %>% 
  ggplot(aes(participant_age,participant_vocabulary_size)) +
  geom_point() +
  geom_smooth() +
  theme_bw() +
...
d$edu2 = ifelse(as.double(d$edu) > 200, NA, as.double(d$edu))

p2 = d %>% 
  distinct(id,participant_age,participant_vocabulary_size,edu2) %>% 
  ggplot(aes(participant_age,edu2)) +
  geom_point() +
  geom_smooth() +
  theme_bw() +
...
p3 = d %>% 
  distinct(id,participant_age,participant_vocabulary_size,edu2) %>% 
  ggplot(aes(participant_vocabulary_size,edu2)) +
  geom_point() +
  geom_smooth() +
  theme_bw() +
  ylab('years spent in education') +
  xlab('participant vocabulary size (1-50)') +
...
p1 = plot_model(fit15, type = "pred", terms = c("participant_vocabulary_size","word_familiarity [20,30,40]","participant_age [14, 25, 70]")) +
  ...
  xlab('participant vocabulary size (1-50)') +
  ylab('predicted response time (s)')
p1
```
...
p2 = plot_model(fit15, type = "pred", terms = c("participant_age","word_familiarity [20,30,40]","participant_vocabulary_size [15, 35, 47]")) +
  ...
  scale_colour_viridis_d(option = 'viridis') +
  scale_fill_viridis_d(option = 'viridis') +
  labs(colour = 'word familiarity (1-50)') +
  xlab('participant age (years)') +
  ylab('predicted response time (s)')
p2
```
...
Fourth, we also make a plot for word familiarity.


```{r sjplot4, fig.width = 9.5, fig.height = 3}
p4 = plot_model(fit15, type = "pred", terms = c("word_familiarity","participant_vocabulary_size [15, 35, 47]","participant_age [14, 25, 70]")) +
  theme_bw() +
  scale_colour_viridis_d(option = 'rocket') +
  scale_fill_viridis_d(option = 'rocket') +
  labs(colour = 'participant\nvocabulary size') +
...
```{r sjplots, eval=F}
p1 + (p2 + theme(legend.position = 'none') + ggtitle('')) + (p3 + ggtitle('')) + (p4 + theme(legend.position = 'none') + ggtitle('')) + plot_layout(ncol = 1, guides = 'collect') + plot_annotation(tag_levels = 'A')
ggsave('figures/sjplots.png', width = 10, height = 12)
ggsave('figures/sjplots.pdf', width = 10, height = 12)
```
